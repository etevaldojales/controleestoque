...
if(is_array($pedido)) {
	...
	$data 		= date("Y/m/d");
	$idpedido = $_pedido->insert($idcli,$numpedido,$data,0, 0, 0, 1, '', $_SESSION["usuario"]);
}
                      ...
                      <h4>&nbsp;</h4>
                        <table class="table table-hover invoice-input">
                            ...
                            if(is_array($itens)) {
								...
								<tr>
									<td>
                                    	<input type="number" class="input-mini" name="qtdi" id="qtdi<?=$x?>" value="<?=$i['quantidade']?>" style="width:50px;" 
                                        onKeyDown="capturarQtd(event, <?=$i['id']?>, this.value)" onclick="this.value=''">
                                    </td>
									<td><?=$produto['nome']?></td>
                                    ...
                                    <td>
                                        <button data-placement="left" class="btn btn-danger" onClick="excluirItem(<?=$i['id']?>)"> 
                                	...
                                	<td colspan="9">
                                        <div class="span4 invoice-block pull-right">
                                            <ul class="unstyled amounts">
                                                <li class="total-display"><strong>Total :</strong> R$ <?=number_format($total,2,",",".")?></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
                                <tr>
                                	<td colspan="9">
                                        ...
                                        <div class="row-fluid text-center">
                                            ...
                                            onClick="comprar()">Retornar a comprar</a>
                                        </div>
                                        </nobr>
                                    </td>
                                </tr>
