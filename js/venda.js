  ...
  if (xhReq.readyState == 4) {
    ...
    if (xhReq.responseText != "") {
      ...
      if (
        ...
        alert("Item já cadastrado nesse pedido");
      } else {
    ...
    $.ajax({
      ...
      success: function (r) {
        ...
        if (r && r.id) {
          ...
          var unidade = r.unidade == 1 ? "Valor unitário:" : "Valor Kg:";
...
function capturarQtd(event, iditem, qtd) {
  ...
  if (window.event) {
    ...
    // Netscape/Firefox/Opera AQUI ESTAVA O PEQUENINO ERRO ao inves de "e." é "event."
    keynum = event.which;
  }
  if (keynum == 13) {
    // 13 é o codigo do Enter --> AQUI TAMBEM
...
function capturarIpi(event, iditem, ipi) {
  ...
  if (window.event) {
    ...
    // Netscape/Firefox/Opera AQUI ESTAVA O PEQUENINO ERRO ao inv�s de "e." � "event."
    keynum = event.which;
  }
  if (keynum == 13) {
    // 13 � o c�digo do Enter --> AQUI TAMBEM
...
function gerarEnviarNfe(orderId) {
  $.ajax({
    ...
    error: function (xhr, status, error) {
      alert("Erro na requisição NFe: " + error);
    },
  });
}
...
function validarRe() {
  if (xhReq.readyState == 4) {
    ...
    // se o retorno for um XML, ent�o come�a a agir.
    if (retorno != null) {
  ...
  if (xhReq.readyState == 4) {
    if (xhReq.responseText != "") {
      ...
      if (
        ...
        alert(
          "O valor de venda n�o pode ser menor ou igual ao valor de compra"
        );
        if (document.getElementById("btnCloseModal")) {
          document.getElementById("btnCloseModal").click();
        }
...
function cadastrarEstoqueRapido() {
  ...
  if (!qtdentrada || !estoquemin) {
    alert("Por favor, insira a quantidade de entrada e o estoque mínimo.");
    return false;
  }
